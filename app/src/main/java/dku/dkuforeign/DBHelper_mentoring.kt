...
class DBHelper_Mentor(context : Context) : SQLiteOpenHelper(context, "Mentor.db", null, 1){
    override fun onCreate(db: SQLiteDatabase?) {
        var sql = "create table MentorTable ( " +
                "idx integer primary key autoincrement, " +
                "titleData text not null, " +
                "nameData text not null, " +
                "contentData text not null, " +
                "nationData text not null, " +
...
class DBHelper_Mentee(context : Context) : SQLiteOpenHelper(context, "Mentee.db", null, 1){
    override fun onCreate(db: SQLiteDatabase?) {
        var sql = "create table MenteeTable ( " +
                "idx integer primary key autoincrement, " +
                "titleData text not null, " +
                "nameData text not null, " +
                "contentData text not null, " +
